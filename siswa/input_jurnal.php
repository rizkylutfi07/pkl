...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Proses upload foto
    $target_dir = "../uploads/";
    $foto_name = basename($_FILES["foto_kegiatan"]["name"]);
    ...
    if ($uploadOk == 0) {
        echo "Maaf, file tidak ter-upload.";
    } else {
        if (move_uploaded_file($_FILES["foto_kegiatan"]["tmp_name"], $target_file)) {
            // Simpan data siswa beserta nama file foto di database
            // Insert data ke database
            $query = "INSERT INTO tb_jurnal (id_jurnal, nisn, nama_siswa, tahun_pelajaran, tanggal, evadir_personal, evadir_sosial, foto_kegiatan, deskripsi_kegiatan, lokasi, id_guru, id_dudi, semester, nama_dudi, nama_guru)
                        VALUES ('$id_jurnal','$nisn', '$nama_siswa', '$tahun_pelajaran', '$tanggal', '$evadir_personal', '$evadir_sosial', '$foto_name', '$deskripsi_kegiatan','$lokasi', '$id_guru', '$id_dudi', '$semester', '$nama_dudi', '$nama_guru')";

            if (mysqli_query($koneksi, $query)) {
                echo "<script>alert('Data berhasil disimpan'); window.location.href='data_jurnal.php';</script>";
            } else {
                echo "<script>alert('Gagal menyimpan data siswa!');</script>";
            }
        } else {
            echo "<script>alert('Maaf, terjadi kesalahan saat meng-upload file!');</script>";
        }
    }
}
...
<body class="bg-light">
    ...
    <div class="container">
        ...
        <form method="POST" action="" enctype="multipart/form-data" onsubmit="return getLocation()">
            ...
            <div class="mb-3">
                <label for="deskripsi_kegiatan" class="form-label">Nama Siswa :</label>
                <input type="text" id="nama_siswa" name="nama_siswa" class="form-control" value="<?php echo $siswa['nama_siswa']; ?>" readonly required>
            </div>

            <div class="mb-3">
                <label for="deskripsi_kegiatan" class="form-label">Tempat PKL :</label>
                <input type="text" id="nama_dudi" name="nama_dudi" class="form-control" value="<?php echo $siswa['nama_dudi']; ?>" readonly required>
            </div>

            <div class="mb-3">
                <label for="deskripsi_kegiatan" class="form-label">Tanggal :</label>
            ...
            <input type="text" id="id_dudi" name="id_dudi" class="form-control" value="<?php echo $siswa['id_dudi']; ?>" readonly required hidden>
            <input type="text" id="semester" name="semester" class="form-control" value="<?php echo $siswa['semester']; ?>" readonly required hidden>
            <input type="text" id="id_guru" name="id_guru" class="form-control" value="<?php echo $guru['id_guru']; ?>" readonly required hidden>
            <input type="text" id="nama_guru" name="nama_guru" class="form-control" value="<?php echo $guru['nama_guru']; ?>" readonly required hidden>
